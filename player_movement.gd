...
var face_direction
var allow_facing:bool = true

# Get the gravity from the project settings to be synced with RigidBody nodes.
var gravity = ProjectSettings.get_setting("physics/3d/default_gravity")

func _ready():
	EventManager.add_listener(str(EventManager.EVENT_NAMES.ON_ENABLE_FACING),self,"_set_facing")
func _set_speed(value: float):
	speed = value
func _set_dash_multiplier(value: float):
	dash_multiplier = value
func _set_facing(params:Array):
	allow_facing = params[0]
...
# sends to animation fo reflect movement
	if(animation_component != null && allow_facing):
